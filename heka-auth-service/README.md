...
| `DB_PASSWORD` | `heka1`             | Database password. **Replace in any non-trivial deployment** — a startup warning is logged while the default is used; with `NODE_ENV=production` the service refuses to start. |
...
| `JWT_SECRET`                | `test`                  | HMAC secret used to sign tokens. **Replace in any non-trivial deployment** — a startup warning is logged while the default is used; with `NODE_ENV=production` the service refuses to start. |
...
| Variable   | Default   | Description                                                              |
|------------|-----------|--------------------------------------------------------------------------|
| `NODE_ENV` | _(unset)_ | When set to `production`, switches the logger to non-pretty JSON output and refuses to start while `JWT_SECRET` or `DB_PASSWORD` are unset or left at their defaults. |
