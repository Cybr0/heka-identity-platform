...
| `DB_PASSWORD` | `heka1`             | Database password. **Replace in any non-trivial deployment** — a startup warning is logged while the default is used; with `NODE_ENV` set to anything other than `development` or `test` (e.g. `production`, case-insensitive) the service refuses to start. |
...
| `JWT_SECRET`                | `test`                  | HMAC secret used to sign tokens. **Replace in any non-trivial deployment** — a startup warning is logged while the default is used; with `NODE_ENV` set to anything other than `development` or `test` (e.g. `production`, case-insensitive) the service refuses to start. |
